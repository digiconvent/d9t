package iam_service_test_utils
...
func GetTestIamService() *iam_service.IamServices {
	...
	environment.Env = env
	dbPath := path.Join(os.TempDir(), "digiconvent_test", "iam", "iam.db")
	if !ffs.Exists(dbPath) {
		panic("Database for testing does not exist: " + dbPath)
	}
	testDb, _ := db.New(dbPath)
